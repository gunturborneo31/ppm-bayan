            ...
            <template v-for="prog in programs" :key="prog.id">
              ...
              >
                ...
                <td class="px-4 py-3 text-center text-xs">
                  <span v-if="k.program?.pilar" class="px-1.5 py-0.5 rounded-lg bg-orange-100 text-orange-700 text-[10px] font-medium">
                    {{ k.program.pilar.nama }}
                  </span>
                ...
                <td class="px-4 py-3 text-center">
                  <div class="flex flex-wrap gap-1 justify-center">
                    ...
                    <button
                      v-if="canResubmit(k)"
                      @click="submitUlang(k)"
                      class="text-[11px] px-2 py-1 rounded border border-amber-300 bg-amber-50 text-amber-700 hover:bg-amber-100 transition-colors"
                    >
                      Ajukan Ulang
                    </button>
                    <button
        ...
        <div class="space-y-3 rounded-xl border border-gray-200 bg-gray-50 p-4">
          <div class="flex items-center justify-between gap-3">
            <div>
              ...
              <p class="text-xs text-gray-500">Setelah deskripsi, tambahkan lokasi pelaksanaan beserta jadwal dan rincian tiap lokasi.</p>
            </div>
...
<button type="button" @click="addLokasiRow" class="px-3 py-2 rounded rounded-xl w-full border border-gray-300 bg-white text-sm font-bold text-gray-800 hover:bg-gray-100 cursor-pointer">+ Tambah Lokasi</button>
        ...
        <div class="flex gap-3 justify-end pt-2">
          ...
          <button type="submit" :disabled="form.processing || isBudgetExceeded" class="px-3 py-2 rounded border border-gray-300 bg-white font-bold text-gray-800 hover:bg-gray-100 disabled:opacity-60 cursor-pointer">
            {{ form.processing ? 'Menyimpan...' : (editing ? 'Simpan' : 'Simpan & Ajukan') }}
          </button>
        </div>
      </form>
    </Modal>
...
const currentUserDivisiName = computed(() => {
  const found = props.divisis.find((divisi) => divisi.id === currentUserDivisiId.value)
  return found ? found.nama : '-'
})
const lokasiDatalistId = 'lokasi-perencanaan-options'
...
function canEdit(k) {
  return !['disetujui', 'diajukan', 'diajukan_ulang'].includes(k.status)
}

function canVerify(k) {
  return ['diajukan', 'diajukan_ulang'].includes(k.status)
}

function canResubmit(k) {
  return ['revisi', 'ditolak'].includes(k.status)
}
