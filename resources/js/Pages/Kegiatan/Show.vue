        ...
        <div class="bg-white rounded-xl shadow-sm border p-6">
          ...
          <div class="mt-6 grid grid-cols-2 md:grid-cols-4 gap-4">
            ...
            <div class="bg-gray-50 rounded-lg p-3">
              ...
              <p class="font-semibold text-gray-800 mt-0.5">{{ fmt(kegiatan.rencana_biaya) }}</p>
            </div>
            <div class="bg-gray-50 rounded-lg p-3">
              <p class="text-xs text-gray-500">Realisasi Output</p>
              <p class="font-semibold text-blue-700 mt-0.5">
                {{ kegiatan.realisasis?.reduce((s,r) => s + Number(r.realisasi_output||0), 0) }}
              </p>
            </div>
            <div class="bg-gray-50 rounded-lg p-3">
              <p class="text-xs text-gray-500">Progress</p>
              <p class="font-semibold text-blue-700 mt-0.5">{{ kegiatan.progress }}%</p>
            </div>
          </div>

          <div class="mt-4">
            <div class="h-3 bg-gray-100 rounded-full overflow-hidden">
              <div class="h-full bg-blue-500 rounded-full transition-all" :style="{ width: kegiatan.progress + '%' }" />
            </div>
          </div>
          ...
          <div v-if="kegiatan.pilars?.length" class="mt-4 flex flex-wrap gap-2">
            <span v-for="p in kegiatan.pilars" :key="p.id"
              class="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full text-xs border border-blue-100">
              {{ p.nama }}
            </span>
          </div>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-sm border">
          ...
          <div class="divide-y">
            <div v-for="f in kegiatan.files" :key="f.id" class="px-4 py-3 flex items-center justify-between">
              ...
              <a :href="route('files.download', f.id)" class="text-blue-600 hover:text-blue-800 text-xs ml-2 flex-shrink-0">
                Unduh
              </a>
            </div>
